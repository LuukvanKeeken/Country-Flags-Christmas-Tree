...
# Set the directory from which to get the flag images.
directory = os.fsencode("images_europe")
# Set the amount of frames per flag.
frames_per_flag = 180
...
# Normalize the y and z coordinates to the range [0, 1].
# This is required for matching each light with the colour
# of its corresponding position on the flag.
y_normalized = (y_positions - y_positions.min())/(y_positions.max() - y_positions.min())
z_normalized = (z_positions - z_positions.min())/(z_positions.max() - z_positions.min())

# Open the csv file. First write a header line. Then loop
# over the images in the directory. 
with open("flags_of_europe_alphabetical.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    ...
    for file in os.listdir(directory):
        ...
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            # Read in the image
            img = np.asarray(image.imread("images/" + filename))
            
            image_height = img.shape[0]
            image_width = img.shape[1] 
            
            # Loop over the lights. For each light, look at its 
            # y and z coordinates, which are normalized to the
            # range [0, 1]. Find the corresponding coordinates
            # on the image. This is done by multiplying the normalized
            # light coordinate with the image height or width, and
            # flooring to make the coordinate an integer.
            colours = []
            for i in range(len(z_positions)):
                z_in_img = math.floor(image_height - (z_normalized[i]*image_height))
                y_in_img = math.floor(y_normalized[i]*image_width)

                # Fix mistakes in calculations.
                if z_in_img < 0:
                    z_in_img = 0
                if z_in_img >= image_height:
                    z_in_img = image_height - 1
                if y_in_img < 0:
                    y_in_img = 0
                if y_in_img >= image_width:
                    y_in_img = image_width - 1
                
                # Append the RGB values of this pixel in the image
                # to the list of light colours.
                colours.append(img[z_in_img][y_in_img][:3])

            # Transform the RGB values from the range [0, 1] to [0, 255]
            colours = np.asarray(colours)
            colours *= 255
            # Set the first value to the frame number. Then add all
            # RGB values for each light to the list of colours.
            colour_row = [counter]
            for i in range(len(colours)):
                for j in range(3):
                    colour_row.append(colours[i][j])

            # For a set amount of frames, add the list of colours to the csv file.
            for i in range(frames_per_flag):
                writer.writerow(colour_row)
                counter += 1
                colour_row[0] = counter
